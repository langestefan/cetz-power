...
#diagram(length: 1cm, {
  ...
  let station(tag, y) = {
    wire(P(35, y - 16), P(67, y - 16))
    wire(P(35, y + 16), P(67, y + 16))
    wire(P(67, y), P(105, y))
    wire(P(180, y), P(224, y))
  ...
  let xsec(tag, cx, cy, cores, aux: false) = {
    ...
    for (i, core) in cores.enumerate() {
      ...
      if core.len() > 1 {
        cetz.draw.content(P(cx + dx, cy + dy), text(
          size: 7pt,
          style: "italic",
          fill: core.at(2),
        )[#core.at(1)])
    ...
    if aux {
      ...
      for (i, h) in hulp.enumerate() {
        cetz.draw.circle(
          P(cx + h.at(0).at(0), cy + h.at(0).at(1)),
          radius: 7 * s,
          stroke: 0.7pt + black,
          fill: h.at(1),
        )
  ...
  let fases = (
    (cred, [fase], white),
    (cyel, [fase], black),
    (nul-fill, [nul], blue.darken(35%)),
    (cblu, [fase], white),
  )
  let caption(y, c) = cetz.draw.content(P(365, y), text(weight: "bold")[#c])
  let subfig(y, c) = cetz.draw.content(P(82, y), text(
    size: 10pt,
    style: "italic",
  )[#c])
  ...
  cetz.draw.line(P(378, 589), P(448, 589), stroke: (
    dash: "dashed",
    thickness: 0.8pt,
    paint: black,
  ))
  ...
  caption(
    755,
    [Figuur 2.28 Aansluiting op een OV-hoofdkabel met aftaksysteem en met rijgsysteem],
  )
})
