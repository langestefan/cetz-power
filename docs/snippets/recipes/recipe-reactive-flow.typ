...
#diagram(length: 1cm, {
  ...
  let qpair(cx, y) = {
    cetz.draw.line((cx - 0.5, y + 0.62), (cx + 0.5, y + 0.62),
  ...
  external-grid("hv", (x-grid, ym), angle: 90deg, size: 0.8,
    background: luma(190))
  ...
  for (y, k) in ((yt, "t"), (ym, "m"), (yb, "b")) {
    transformer("hvmv-" + k, (x-td, y), (hv-out, y), radius: tr-r, distance: tr-d)
  ...
  cetz.draw.line((2.85, 0.85), "hvmv-t",
    mark: (end: ">", fill: black), stroke: 0.7pt + black)

  // ── MV/LV transformers (MV bus → LV feeders) ────────────────────
  for (y, k) in ((yt, "t"), (ym, "m"), (yb, "b")) {
    transformer("mvlv-" + k, (x-mv, y), (lv-in, y), radius: tr-r, distance: tr-d)
  }
  for y in (yt, yb) {
  ...
  dali("dali", (9.1, ym), width: 0.55, lead: 0.12, tail: 0.26,
    box-width: 0.95, box-height: 0.5,
    clamp-radius: 0.09, tx-radius: 0.1, tx-distance: 0.1)
  ...
  let lx = 9.4
  let ly = -1.85
  cetz.draw.line((lx, ly), (lx + 0.8, ly),
    mark: (end: ">", fill: black), stroke: 0.8pt + black)
  cetz.draw.content((lx + 0.95, ly), anchor: "west", [$Q_1$ TSO to DSO])
  cetz.draw.line((lx + 0.8, ly - 0.42), (lx, ly - 0.42),
    mark: (end: ">", fill: black, stroke: 0.8pt + black),
    stroke: (paint: black, thickness: 0.8pt, dash: "dashed"))
