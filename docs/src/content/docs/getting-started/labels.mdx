...
```typst
// Plain content uses the automatic position — the symbol's free side
// (north beside this horizontal run).
transformer("t1", (0, 0), (2, 0), label: [132/11 kV])

// Dict form overrides any of: content, anchor, align, distance, size, upright.
transformer("t2", (0, -1), (2, -1), label: (
  ...
  size: 9pt,          // font size
  upright: false,     // `true` turns an east/west label to read along a vertical run
))
```

`anchor:` defaults to `auto`, which picks the symbol's **free side**:
the perpendicular of the in→out axis on a two-node symbol (north
beside a horizontal run, east beside a vertical one), the symbol's own
hint on a one-node symbol (e.g. beside a load's arrow tip, rotated
along with `angle:`), else `"north"`. See
[Automatic placement](/getting-started/auto-placement/) for the full
rules and how to disable them.

The four transformers below show how `anchor:`, `distance:`, and a
non-compass anchor name change where the text lands:

<Snippet name="getting-started-labels-anchors" />
...
| Layer            | What it sets                                                 |
| ---------------- | ------------------------------------------------------------ |
| Base default     | `anchor: auto`, `align: auto`, `distance: 0.15`, `size: 8pt` |
| Bus family       | `distance: 0.22` (overrides only the gap)                    |
| Per-call         | Whatever you pass in `label: (...)`                          |
| Runtime override | `cetz.draw.set-style(cetz-power: (bus: (label: (...))))`     |
...
Setting a family-level default and overriding it per call:

<Snippet name="getting-started-labels-cascade" />

`b1` and `b2` inherit the family default (`anchor: "south"`); `b3`
passes its own `anchor: "north"`, which wins.
...
opposite of `anchor:` so the text reads cleanly _away from_ the symbol
