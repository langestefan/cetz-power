// Mixer / multiplier — the control-block-diagram circle with an
// inscribed "×" (the diagonals span the full circle). Used for
// multiplication, modulation and mixing points in control schemes.

#import "/src/deps.typ": cetz
#import "/src/core.typ": symbol

/// Mixer (circle with an inscribed ×).
///
...
#let mixer(name, ..args) = {
  ...
  let draw(ctx, positions, style) = {
    ...
    cetz.draw.anchor("north-east", (d, d))
    cetz.draw.anchor("south-east", (d, -d))
    cetz.draw.anchor("south-west", (-d, -d))
    cetz.draw.anchor("north-west", (-d, d))
  }

  symbol("mixer", name, ..positions, ..overrides, draw: draw)
}
