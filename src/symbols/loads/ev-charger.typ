...
#let ev-charger(name, ..args) = {
  ...
  if on != none or towards != none {
    assert(
      ...
      message: "ev-charger(): pass `on:` or `towards:`, not both",
    )
    assert(
      positions.len() == 1,
      message: "ev-charger(): `on:`/`towards:` need exactly one position",
    )
    assert(
      "angle" not in overrides,
      message: "ev-charger(): `angle:` conflicts with `on:`/`towards:`",
    )
    return cetz.draw.get-ctx(ctx => {
      let st = resolve-style(ctx, "ev-charger", overrides)
      ...
      ev-charger(name, positions.first(), ..overrides, ..fwd)
    })
  }
  assert(side == auto, message: "ev-charger(): `side:` requires `on:`")
