...
- `src/symbols/loads/` — energy consumers (`load`, `factory`, `ev-charger`). `factory` is the industrial-consumer building pictogram (sawtooth roof with `teeth:` teeth, a steam pipe standing on the roof-base ledge, `windows:` window dashes, optional `smoke: true` wisps); `ev-charger` is the charging-station pictogram (`kind:` `"ev-charger"` car + pedestal + cable into the plug on the car's rear panel, `"charger"`, or `"ev"`). Both sit in a `plant`/`block`-style device box by default (square for `ev-charger`; `box: false` for the bare pictogram); the lead lands on the box top (bare: pipe/pedestal top) and follows `load`'s lead/`elbow:` conventions.
...
Every symbol under `src/symbols/<category>/<name>.typ` is a thin closure around `symbol()` in `src/core.typ`. `symbol(family, name, ..positions, draw: <closure>, label:, label-dir:, angle:)` does the heavy lifting:
...
4. **Labels** are drawn *outside* the rotated CeTZ group (in world frame) so text stays upright regardless of symbol rotation. The `anchor:` in the label dict is a **world-space** compass direction; `core.typ` rotates it back through `-effective-angle` to find the matching local anchor on the symbol. The default is `anchor: auto` — the symbol's **free side**: a two-node symbol labels on the perpendicular of its in→out axis (north beside a horizontal run, east beside a vertical one); a one-node symbol uses the `label-dir` hint its wrapper passed to `symbol()` (a LOCAL-frame angle, e.g. `-90deg` for a load's tip side, rotated along with `angle:`), or "north" without one. Auto snaps to the four cardinals only. The label-dict key `upright: true` turns a label −90° when it lands east/west. Families that pin an explicit `anchor:` in `styles.typ` (machine, vt, bolt, autotransformer, block/plant/converter, transformer3, …) bypass auto entirely. Geometry helpers for all of this (compass snapping, `perp-of`, `opposite`, `upright`) live in `src/layout.typ` — pure functions, importable from both `symbols/` and `helpers/` without cycles.
5. After drawing, the CeTZ "pen" is moved to the last input position so chained `line((), (rel: ...))` calls behave naturally.

**Context-aware drops.** `load`, `factory`, `ev-charger` and `pv-panel` also accept `on: "<bus-name>"` — the drop orients itself from that bus's axis (straight below a horizontal bar, `side: "north"` above; automatic rule-8 L-bend off a vertical bar via the family `elbow` style key, `side: "east"`/`"west"`) — and `towards: <coord>`, which aims the tip at a coordinate. Both conflict with `angle:` and require the bus to be drawn first (`utils.typ::drop-orientation`). Default load geometry is the empirically-good small form (`size 0.18`, `lead 0.15`); the two-lead passives take a `lead:` shorthand that sets `lead-in`/`lead-out` together.
...
**Aligning bus tops/ends — use `fit:`.** When a bar must span a set of connections with a clean symmetric overshoot (and possibly line its ends up with a shorter reference bar), don't hand-derive endpoints: `bus("b", fit: (c1, c2, ...), over: 0.2)` spans the given coordinates plus `over` at *both* ends, and `over: (ref: h, gap: g)` computes the rule-14 alignment overshoot `h/2 − g/2` for you. The bar's start→end direction follows the order of the fit points (so `bus-frac` fractions keep their orientation), the axis is inferred from the points' spread (forced with `angle:`), and the fit anchors must already be drawn. The old manual pattern (`over = ref-h/2 - gap/2` applied to both ends) is what `fit:` encodes — never extend only one end.

### Wires are not symbols

`wire()` and `elbow()` in `src/symbols/grid/wire.typ` skip the `symbol()` machinery (no family-style cascade beyond `cetz-power.wire.stroke`). Both take `kind: "cable"` to re-dash the resolved stroke (pattern from `cetz-power.wire.cable-dash`) so underground cables read differently from overhead lines. `wire(..., label: <content>)` drops a caption at the polyline midpoint: `label-side:` defaults to `auto` (the wire's perpendicular — north above a horizontal wire, east beside a vertical one, where `label-upright: auto` also turns the text to read along the conductor). For captions elsewhere on a run use `note(a, b, body)` — the segment form of `note` with the same auto side/upright behavior and an `at:` fraction. `elbow(a, b, corner:, stub: d)` with a non-zero `stub` draws the rule-4 join (perpendicular stub → diagonal → equal stub) instead of a square corner.

### Composition helpers

`src/helpers/` (one file per helper, mirroring `src/symbols/`) is for short combinations of existing primitives that would otherwise force the caller to write the same loop or coordinate math repeatedly — it is **not** for new symbols. `src/lib.typ` re-exports each helper. It currently exports ten things:

- `link(a, b, from:, to:, side:, clearance:, label:, kind:, stroke:)` — **the** way to connect two named buses: taps each bar's *interior* (`from:`/`to:` fractions, default centre) and leaves it perpendicular, routing straight/L/Z/U legs as the bars' geometry requires (facing parallel → straight, perpendicular → L, offset parallel → Z, collinear → U on `side:` with `clearance:`). `label:` captions the longest leg via `note`'s segment form. Never hand-route a bar-to-bar `wire` — endpoint landings (`wire("a.end", "b.start")`) are exactly the mistake `link` exists to prevent.
...
- `note(pos, body, side:)` / `note(a, b, body, side:, at:, upright:)` — drops a free-floating text label beside any coordinate/anchor/lerp (point form), or captions the segment `a`→`b` (segment form): the label sits `at:` (default 50 %) of the way along, `side: auto` resolves to the segment's perpendicular, and `upright: auto` turns the text to read along vertical segments. Use it for captions on runs and tap points.
...
- `pixel-map(scale, height: H)` — returns the `P(x, y) = (x·scale, (H − y)·scale)` closure used to digitise a reference image when replicating a published figure (origin top-left, y flipped); `scale` doubles as the compression knob. Every figure-replication recipe opens with it.
