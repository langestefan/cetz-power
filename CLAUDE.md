...
`src/helpers/` (one file per helper, mirroring `src/symbols/`) is for short combinations of existing primitives that would otherwise force the caller to write the same loop or coordinate math repeatedly — it is **not** for new symbols. `src/lib.typ` re-exports each helper. It currently exports six things:
